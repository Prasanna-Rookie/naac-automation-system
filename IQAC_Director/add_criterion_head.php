...
require '../config.php';
$email = $email_err = $name = $tname = $name_err = $criterion = $criterion_err = $password = $password_err = $cpassword = $cpassword_err = "";
if($_SERVER["REQUEST_METHOD"] == "POST")   	
{
	...
	else 
	{
		...
		{
			$sql = "SELECT criterion_id FROM criterion_head WHERE email = ?";
    ...
    if (empty(trim($_POST["criterion"]))) 
    {
        $criterion_err = "Please Select Criterion.";
    }
    else
   	{
	...
	if(empty($name_err) && empty($email_err) && empty($criterion_err) && empty($password_err) && empty($cpassword_err))
	{
		$hash_password = password_hash($password, PASSWORD_DEFAULT);

		$query = "insert into criterion_head (name, criterion, email, password) values ('$name', '$criterion', '$email', '$hash_password')";

		if(mysqli_query($con, $query))
        {
            echo '<script language="javascript">';
            echo'alert("Criterion Head Added Successfully."); location.href="add_criterion_head.php"';
            echo '</script>';
        }
        else
        {
            echo '<script language="javascript">';
           	echo'alert("Failed, Please Try Again."); location.href="add_criterion_head.php"';
            echo '</script>';
        }
        mysqli_close($con);
	}
                ...
                <div class="container-fluid">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Criterion Head</li>
                        </ol>
                    </nav>
                </div>
                        ...
                        <div class="card-body">
                        	<ul class="nav nav-tabs">
	                    		<li class="nav-item" role="presentation">
	                        		<a class="nav-link active" href="add_criterion_head.php" id="active">Add Criterion Head</a>
	                    		</li>

	                    		<li class="nav-item" role="presentation">
	                        		<a class="nav-link" href="manage_criterion_head.php" id="non-active">Manage Criterion Head</a>
	                    		</li>
	                		</ul>
										...
										<div class="col-sm-6 col-sm-6">
											<div class="card" style="border-top:2px solid #087ec2;">
												<div class="card-header" style="color:#087ec2; font-weight:bold;">
		                            				...
		                            				<span>Add New Criterion Head</span>
		                        				</div>	
		                        				<div class="card-body">
		                                			...
		                                			<div class=" mb-3">
                    									...
                    									<select class="form-select <?php echo (!empty($criterion_err)) ? 'is-invalid' : ''; ?>" id="criterion" name="criterion">

                        									<option selected value="">Select Criterion</option>

									                        <option value="Criterion - 1"
