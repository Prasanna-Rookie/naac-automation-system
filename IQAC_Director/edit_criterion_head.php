...
if(isset($_POST['update']))
{
	...
	if(empty(trim($_POST["name"])))
	{
        ...
        echo'alert("Please enter a Name."); location.href="manage_criterion_head.php"';
        echo '</script>';
	} 
	elseif(!preg_match("/^[a-zA-Z-'. ]*$/",trim($_POST["name"])))
	{
		echo '<script language="javascript">';
        echo'alert("Only letters, white space and dots are allowed."); location.href="manage_criterion_head.php"';
        echo '</script>';
	} 
	else
	{
		...
		$query = "UPDATE criterion_head SET name = '$name' WHERE criterion_id = '$id'";

        if(mysqli_query($con, $query))
        {
            echo '<script language="javascript">';
        	echo'alert("Update Successfully."); location.href="manage_criterion_head.php"';
        	echo '</script>';
        }
        else
        {
            echo '<script language="javascript">';
       		echo'alert("Failed, Please Try Again."); location.href="manage_criterion_head.php"';
       		echo '</script>';
        }
        mysqli_close($con);
	}
...
<div class="table-responsive">
    <table class="table table-bordered" id="table">
        ...
        <tbody>
        	<?php
        		require '../config.php';
				$sql = "SELECT * FROM criterion_head";
				$res = $con->query($sql);
				...
				while($row = $res->fetch_assoc())
				{
					...
					echo"<td><button type='button' class='btn btn-sm btn-success edit' data-id={$row["criterion_id"]}><i class='fa-solid fa-pen-to-square'></i></td>";
