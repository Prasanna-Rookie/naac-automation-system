...
require '../../config.php';
$academic_year = $academic_year_err = $paper_title = $paper_title_err = $author_name = $author_name_err = $journal_titile = $journal_titile_err = $month_year = $month_year_err = $citation_index = $citation_index_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if(empty(trim($_POST["citation_index"])))
	{
		$citation_index_err = "Please Enter  Citation Index.";
	}
	else
	{
		$citation_index = trim($_POST["citation_index"]);
	}

	// Insert
	if(empty($academic_year_err) && empty($paper_title_err) && empty($author_name_err) && empty($journal_titile_err) && empty($month_year_err) && empty($citation_index_err))
	{
		$query = "INSERT INTO cri_3_4_5_citation_index (academic_year, upload_by_id, upload_by_name, paper_title, author_name, journal_titile, month_year, citation_index) VALUES ('$academic_year', '$upload_by_id', '$upload_by_name','$paper_title', '$author_name', '$journal_titile', '$month_year', '$citation_index')";
		if(mysqli_query($con, $query))
		{
			echo '<script language="javascript">';
			echo'alert("Uploaded Successfully."); location.href="cri_3.4.5_data_entry.php"';
			echo '</script>';
		}
		else
		{
			echo '<script language="javascript">';
			echo'alert("Upload Failed, Please Try Again."); location.href="cri_3.4.5_data_entry.php"';
			echo '</script>';
		}
...
{
	$err = 0;
	$paper_title = $title = $journal_titile = $month_year = $citation_index = "";
	if(empty(trim($_POST["paper_title"])) || empty(trim($_POST["author_name"])) || !is_numeric(trim($_POST["month_year"])) || empty(trim($_POST["month_year"])) || empty(trim($_POST["journal_titile"])) || empty(trim($_POST['citation_index'])))
	{
		$err++;
		echo '<script language="javascript">';
        echo'alert("Please Fill the all Fields."); location.href="cri_3.4.5_data_entry.php"';
        echo '</script>';
	}
	else
	{
		if($err == 0)
		{
			...
			$query = "UPDATE cri_3_4_5_citation_index SET paper_title = ?, author_name = ?, journal_titile = ?, month_year = ?, citation_index = ? WHERE ci_id = ?";
			$stmt = mysqli_prepare($con, $query);
			mysqli_stmt_bind_param($stmt, "sssssi", $paper_title, $author_name, $journal_titile, $month_year,$citation_index, $id);
			if(mysqli_stmt_execute($stmt)) 
			{
			    echo '<script language="javascript">';
			    echo 'alert("Update Successfully."); location.href="cri_3.4.5_data_entry.php"';
			    echo '</script>';
			} 
			else 
			{
			    echo '<script language="javascript">';
			    echo 'alert("Update Failed, Please Try Again."); location.href="cri_3.4.5_data_entry.php"';
			    echo '</script>'; 
			}
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-light" style="border-top:3.5px solid #087ec2;">
                        <h6 class="modal-title" id="exampleModalLabel" style="color:#057EC5;"><i class="fa fa-edit"></i><spam>Citation Index</spam></h6>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label class="form-label" style="font-size: 17px; color:dimgrey; font-weight: bold;" for = "citation_index"> <span style="color: red">* </span>Citation Index</label>

                            <input type="number" name="citation_index" class="form-control " id="citation_index">
                        </div>
				...
				<div class="container-fluid">
					<div class="card" style="border-top:2px solid #087ec2;">
						...
						<div class="card-header" style="color:#087ec2; font-weight:bold;">
							...
							<span>Criteria 3.4.5 - Bibliometrics of the publications during the year based on average Citation Index in Scopus/ Web of Science/PubMed.</span>
						</div>
						...
						<div class="card-body">
							<ul class="nav nav-tabs">
								...
								<li class="nav-item" role="presentation">
									<a class="nav-link active" href="cri_3.4.5_data_entry.php" id="active">Citation Index</a>
								</li>

								<li class="nav-item" role="presentation">
									<a class="nav-link" href="cri_3.4.5_doc_upload.php" id="non-active">Upload Document</a>
								</li>

								<li class="nav-item" role="presentation">
									<a class="nav-link" href="cri_3.4.5_doc_view.php" id="non-active">View Document</a>
								</li>
										...
										<div class="row justify-content-center">
											<div class="col-sm-6 col-sm-6">
												<div class="card" style="border-top:2px solid #087ec2;">
													<div class="card-header" style="color:#087ec2; font-weight:bold;">
														<i class="fa-solid fa-trophy fa-lg"></i>
														<span>Criteria 3.4.5 - Citation Index</span>
													</div>
													<div class="card-body">
														<div class="mb-3">
											...
											<div class="col-sm-6 col-sm-6">
												<div class="card" style="border-top:2px solid #087ec2;">
													...
													<div class="card-body">
														...
														<div class="mb-3">
															<label class="form-label" for = "citation_index"> <span style="color: red">* </span>Citation Index</label>

															<input 
															type="number" 
															name="citation_index" 
															class="form-control <?php echo (!empty($citation_index_err)) ? 'is-invalid' : ''; ?>"
															id = "citation_index" 
															value = "<?php echo $citation_index; ?>">

															<div class="invalid-feedback">
																<?php echo $citation_index_err; ?>
															</div>
														</div>
							...
							<?php
							$sql = "SELECT * FROM cri_3_4_5_citation_index WHERE upload_by_id = '$upload_by_id'";
							$datas = $con->query($sql);
							if($datas->num_rows>0)
							{
								?>
										...
										<tbody class="row_position">
											...
											while ($data = $datas->fetch_assoc()) 
											{
												...
												<tr style="text-align:center;">
													...
													<td><?php echo $data['citation_index']; ?></td>
													<?php
													echo"<td><button type='button' class='btn btn-sm btn-success edit' data-id={$data["ci_id"]}><i class = 'fa fa-edit'></i></button></td>";
													?>
												</tr>
												<?php
											}
