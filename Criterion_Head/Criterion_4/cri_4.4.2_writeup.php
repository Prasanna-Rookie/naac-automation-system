...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if(empty($academic_year_err) && empty($writeup_err))
	{
		...
		if($id == 0)
		{
			$criteria = "Metric 4.4.2";
			// Prepare the SQL statement
			$query = "INSERT INTO cri_4_write_up (academic_year, criteria, write_up) VALUES (?, ?, ?)";
			...
			if (mysqli_stmt_execute($stmt)) {
			    ...
			    echo 'alert("Write Up Updated Successfully."); location.href="cri_4.4.2_writeup.php"';
			    echo '</script>';
			} else {
			    echo '<script language="javascript">';
			    echo 'alert("Write Up Update Failed, Please Try Again."); location.href="cri_4.4.2_writeup.php"';
			    echo '</script>';
			}
		...
		{
			// Prepare the SQL statement
			$query = "UPDATE cri_4_write_up SET write_up = ? WHERE id = ?";
						...
						<div class="card-body">
							<ul class="nav nav-tabs">
								...
								<li class="nav-item" role="presentation">
	                                <a class="nav-link active" href="cri_4.4.2_writeup.php" id="active">Write Up</a>
	                            </li>
	...
	<script>
	  $(document).ready(function(){
	     $('#academic_year').change(function(){
	      ...
	      var criteria = "Metric 4.4.2"; 
	 
	      $.ajax({
	        type: 'POST',
	        url: 'cri_4_writeup_fetch.php',
	        data: {
